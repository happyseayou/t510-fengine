...
DAC_COMPLEX_INPUT_RATE_HZ = 320_000_000.0
DAC_SAMPLE_RATE_HZ = DAC_COMPLEX_INPUT_RATE_HZ
RF_FIRST_NYQUIST_MIN_MHZ = 1.0
RF_FIRST_NYQUIST_MAX_MHZ = 1920.0
...
class RfdcCenterConflict(RuntimeError):
    """The DAC request uses a center other than the live RFDC mixer center."""

    def __init__(self, requested_mhz: float, actual_mhz: float) -> None:
        self.requested_mhz = float(requested_mhz)
        self.actual_mhz = float(actual_mhz)
        super().__init__(
            "RFDC_CENTER_CONFLICT: requested center_mhz "
            f"{self.requested_mhz:.9f} does not match the live RFDC mixer center "
            f"{self.actual_mhz:.9f} MHz"
        )
...
class FEngineMode(str, Enum):
    TIME_ONLY = "time_only"
    SPEC_ONLY = "spec_only"
    TIME_SPEC = "time_spec"

    @classmethod
    def parse(cls, value: str | "FEngineMode") -> "FEngineMode":
...
class DacChannelConfig:
    ...
    enabled: bool = True
    rf_frequency_mhz: float = 200.010
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or not RF_FIRST_NYQUIST_MIN_MHZ <= frequency < RF_FIRST_NYQUIST_MAX_MHZ
        ):
            raise ValueError("DAC rf_frequency_mhz must be finite and within 1..1920 MHz (upper bound exclusive)")
...
@dataclass(frozen=True)
class FEngineConfig:
    sample_rate_msps: int = 160
    mode: FEngineMode | str = FEngineMode.TIME_SPEC
    center_mhz: float = 200.0
    board_id: int = 0
    ...
    def __post_init__(self) -> None:
        sample_rate_msps = int(self.sample_rate_msps)
        mode = FEngineMode.parse(self.mode)
        allowed = {
            (160, FEngineMode.TIME_ONLY),
            (160, FEngineMode.SPEC_ONLY),
            (160, FEngineMode.TIME_SPEC),
            (320, FEngineMode.TIME_ONLY),
            (320, FEngineMode.SPEC_ONLY),
        }
        if (sample_rate_msps, mode) not in allowed:
            raise ValueError(
                "Stage 33 supports 160MS/s TIME_ONLY/SPEC_ONLY/TIME_SPEC and "
                "320MS/s TIME_ONLY/SPEC_ONLY"
            )
        center = float(self.center_mhz)
        center_lower_hz, center_upper_hz = T510FEngine.science_center_bounds_hz(sample_rate_msps)
        center_lower = center_lower_hz / 1.0e6
        center_upper = center_upper_hz / 1.0e6
        if not math.isfinite(center) or not center_lower <= center <= center_upper:
            raise ValueError(
                f"center_mhz must be finite and within {center_lower:g}..{center_upper:g} MHz "
                f"for the {sample_rate_msps} MS/s profile"
            )
        ...
        half_band_mhz = self.sample_rate_hz_for(sample_rate_msps) / 2.0 / 1.0e6
        ...
        for channel, dac in enumerate(dac_channels):
            if not lower <= dac.rf_frequency_mhz <= upper:
                raise ValueError(
                    f"DAC CH{channel} frequency must stay within the {sample_rate_msps} MS/s science "
                    f"Nyquist band {lower:.6f}..{upper:.6f} MHz"
                )
        object.__setattr__(self, "sample_rate_msps", sample_rate_msps)
    ...
    @staticmethod
    def sample_rate_hz_for(sample_rate_msps: int) -> float:
        return 160_000_000.0 if int(sample_rate_msps) == 160 else 320_000_000.0

    @property
    def needs_time(self) -> bool:
        return self.mode in (FEngineMode.TIME_ONLY, FEngineMode.TIME_SPEC)

    @property
    def needs_spec(self) -> bool:
        return self.mode in (FEngineMode.SPEC_ONLY, FEngineMode.TIME_SPEC)
    ...
    def sample_rate_hz(self) -> float:
        return self.sample_rate_hz_for(self.sample_rate_msps)
    ...
    def expected_packet_rates(self) -> dict[str, float]:
        pps = 625_000.0 if self.sample_rate_msps == 160 else 1_250_000.0
...
class FEngineController:
    ...
    def connect(self, *, download: bool = False) -> dict[str, Any]:
        ...
        if version != EXPECTED_CORE_VERSION:
            raise RuntimeError(
                f"Stage 33 requires CORE_VERSION=0x{EXPECTED_CORE_VERSION:08x}; got 0x{version:08x}"
            )
        return status

    def require_core(self) -> T510FEngine:
        if self.core is None:
            raise RuntimeError("connect the Stage 33 production controller first")
        return self.core
    ...
    def _program_destinations(self, config: FEngineConfig) -> list[dict[str, Any]]:
        ...
        if mismatches:
            raise RuntimeError("Stage 33 TX endpoint readback mismatch: " + "; ".join(mismatches))
        return endpoints

    @staticmethod
    def _fallback_source_port(config: FEngineConfig) -> int:
        groups = []
    ...
    @staticmethod
    def _flow_tuple_warnings(config: FEngineConfig, endpoints: list[dict[str, Any]]) -> list[str]:
    ...
    def _write_dac_channels(
        ...
        *,
        ...
        for channel, item in enumerate(channels):
            ...
            if abs(offset_hz) > DAC_COMPLEX_INPUT_RATE_HZ / 2.0:
                raise ValueError(
                    f"DAC CH{channel} frequency is outside center +/-160 MHz: "
                    f"center={center_mhz:.6f} MHz rf={item.rf_frequency_mhz:.6f} MHz"
                )
    ...
    def _live_rfdc_center_mhz(self) -> float:
        """Return the common center of all eight enabled DAC RFDC mixers."""
        core = self.require_core()
        if not hasattr(core, "read_rfdc_mixer_frequencies"):
            raise RuntimeError("RFDC_CENTER_READBACK_UNAVAILABLE")
        readback = core.read_rfdc_mixer_frequencies()
        dac_centers = [
            float(item["frequency_mhz"])
            for item in readback.get("mixers", [])
            if item.get("kind") == "dac"
        ]
        if len(dac_centers) != 8 or any(not math.isfinite(value) for value in dac_centers):
            raise RuntimeError(
                "RFDC_CENTER_READBACK_INCOMPLETE: expected eight finite DAC mixer frequencies, "
                f"got {dac_centers!r}"
            )
        if max(dac_centers) - min(dac_centers) > 1.0e-6:
            raise RuntimeError(f"RFDC_CENTER_READBACK_DIVERGED: {dac_centers!r}")
        return sum(dac_centers) / len(dac_centers)

    def _live_science_sample_rate_msps(self) -> int | None:
        status = self.require_core().read_status()
        sample_rate_msps = int(status.get("science_sample_rate_msps", 0) or 0)
        if sample_rate_msps in (160, 320):
            return sample_rate_msps
        code = int(status.get("science_sample_rate_mode", 0) or 0)
        return {1: 160, 2: 320}.get(code)
    ...
    def prepare(
        ...
        *,
        ...
        observation = core.apply_mts_locked_observation_config(
            ...
            view_bw_hz=config.sample_rate_msps * 1_000_000.0,
            amplitude=0,
            phase_deg=0.0,
        ...
        if (
            ...
            raise RuntimeError(
                "Stage 33 requires a successful configure-time MTS result with call evidence"
            )
        mts_result_id = zlib.crc32(
            json.dumps(mts_payload, sort_keys=True, separators=(",", ":"), default=str).encode("utf-8")
        ) or 1
        if hasattr(core, "persist_mts_result_id"):
            core.persist_mts_result_id(mts_result_id)
        observation["mts_result_id"] = mts_result_id
        science = core.configure_science(
            sample_rate_msps=config.sample_rate_msps,
        ...
        if requested_source != source_readback:
            raise RuntimeError(
                "Stage 33 TX source identity readback mismatch: "
        ...
        if board_id_readback != config.board_id:
            raise RuntimeError(
                "Stage 33 board_id readback mismatch: "
    ...
    def start_immediate(self, *, timeout: float = 2.0) -> dict[str, Any]:
        ...
        if not self._stream_path_accepting(status):
            raise RuntimeError(
                "Stage 33 immediate start did not produce an accepting streaming "
                f"data path: {status}"
            )
        return status
    ...
    def stop_and_verify(self, *, settle_seconds: float = 0.1, timeout: float = 2.0) -> dict[str, Any]:
        ...
        while True:
            ...
            if time.monotonic() >= deadline:
                raise RuntimeError(f"Stage 33 stop could not be proven: {status}")
            previous = status
    ...
    def apply_dac_live(
        ...
        *,
        ...
        ``center_mhz`` is required after reconnecting to an already configured
        overlay when no configuration is cached locally.
        """
        if self.config is None and center_mhz is None:
            raise RuntimeError("center_mhz is required when no Stage 33 configuration is cached")
        channels = tuple(
            item if isinstance(item, DacChannelConfig) else DacChannelConfig(**dict(item))
        ...
        if len(channels) != 8:
            raise ValueError("dac_channels must contain exactly 8 entries")
        requested_center_mhz = (
            float(center_mhz)
            if center_mhz is not None
            else float(self.config.center_mhz)
        )
        live_sample_rate_msps = (
            self.config.sample_rate_msps
            if self.config is not None
            else self._live_science_sample_rate_msps()
        )
        if live_sample_rate_msps in (160, 320):
            lower_hz, upper_hz = T510FEngine.science_center_bounds_hz(live_sample_rate_msps)
            lower_mhz, upper_mhz = lower_hz / 1.0e6, upper_hz / 1.0e6
        else:
            lower_mhz, upper_mhz = 80.0, 1840.0
        if (
            not math.isfinite(requested_center_mhz)
            or not lower_mhz <= requested_center_mhz <= upper_mhz
        ):
            raise ValueError(
                f"center_mhz must be finite and within {lower_mhz:g}..{upper_mhz:g} MHz"
            )
        active_center_mhz = self._live_rfdc_center_mhz()
        if abs(requested_center_mhz - active_center_mhz) > 1.0e-6:
            raise RfdcCenterConflict(requested_center_mhz, active_center_mhz)
        validation_sample_rate_msps = live_sample_rate_msps or 320
        half_band_mhz = validation_sample_rate_msps / 2.0
        for channel, item in enumerate(channels):
            if abs(item.rf_frequency_mhz - active_center_mhz) > half_band_mhz:
                raise ValueError(
                    f"DAC CH{channel} frequency must stay within center +/-{half_band_mhz:g} MHz"
                )
        if self.config is None:
    ...
    def validate(self, *, seconds: float = 10.0) -> dict[str, Any]:
        if self.config is None:
            raise RuntimeError("apply a Stage 33 production configuration first")
    ...
    def capture_preview(self, *, time_window_us: float = 0.25) -> dict[str, Any]:
        ...
        analysis = core.compute_observation_view(
            ...
            view_bw_hz=self.config.sample_rate_msps * 1_000_000.0,
