...
def create_console(project_root: str | Path):
    """Build the current PYNQ production console."""
    ...
    controller = FEngineController(bitfile)
    state: dict[str, Any] = {"live": False, "task": None, "last_capture": None}
    label_style = {"description_width": "115px"}
    wide = W.Layout(width="420px")

    sample_rate = W.Dropdown(
        options=[
            ("160 MS/s（center 80..1840 MHz）", 160),
            ("320 MS/s（center 160..1760 MHz）", 320),
        ],
        value=160,
    ...
    center_mhz = W.FloatText(value=200.0, description="RF center MHz", style=label_style, layout=wide)
    ...
    for channel in range(8):
        dac_rows.append({
            "enabled": W.Checkbox(value=True, indent=False, layout=W.Layout(width="34px")),
            "frequency": W.FloatText(value=200.010, step=0.001, layout=W.Layout(width="130px")),
    ...
    status = W.HTML("<pre>current console ready. Apply + Start performs a fresh download.</pre>")
    ...
    def current_config() -> FEngineConfig:
        return FEngineConfig(
            sample_rate_msps=int(sample_rate.value), mode=str(mode.value), center_mhz=float(center_mhz.value),
    ...
    sample_rate.observe(update_mode_options, names="value")

    def refresh_visibility(config: FEngineConfig) -> None:
    ...
    def compact_board() -> dict[str, Any]:
        ...
        return {
            ...
            "science": {key: science.get(key) for key in ("science_sample_rate_msps", "science_output_mode", "time_enabled", "spec_enabled", "science_antialias_100m_active", "science_antialias_100m_primed", "science_block_reasons")},
    ...
    def show_network_apply(result: dict[str, Any], config: FEngineConfig) -> None:
    ...
    def sync_rust(config: FEngineConfig) -> dict[str, Any]:
    ...
    def render_preview(result: dict[str, Any], config: FEngineConfig) -> None:
    ...
    def apply_clicked(_button: Any) -> None:
        try:
            config = current_config()
            set_status("Fresh download and deterministic current start in progress…")
            ...
            set_status(
                f"Applied board {config.board_id}, {config.sample_rate_msps} MS/s {config.mode.value}; "
    ...
    science_tab = W.VBox([sample_rate, mode, center_mhz, rust_url, source_identity, W.HTML("<small>Sample-rate/center/network changes require Apply + Start. Release gates remain in the current host gate.</small>")])
    ...
    return W.VBox([W.HTML("<h2>current F-engine Production Control</h2>"), status, controls, tabs, health, preview_box])
