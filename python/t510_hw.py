...
CLOCK_PRODUCTION_REFERENCES = {
    "external_10mhz": {
        "driver_ref": "external_10mhz",
...
def _production_clock_selection(body: dict[str, Any]) -> dict[str, str]:
    requested = str(body.get("clock_reference", "external_10mhz")).strip().lower()
    try:
        selection = CLOCK_PRODUCTION_REFERENCES[requested]
    except KeyError as exc:
        raise HelperError(
            "CLOCK_REFERENCE_INVALID",
            "clock_reference must be external_10mhz or onboard_tcxo",
            exit_code=EXIT_INVALID,
        ) from exc
    return {"api_reference": requested, **selection}


def _production_clock_readback_errors(
    selection: dict[str, str], live: dict[str, Any]
) -> list[str]:
    errors: list[str] = []
    if str(live.get("profile_id")) != selection["profile_id"]:
        errors.append("PROFILE_READBACK_MISMATCH")
    if str(live.get("clock_reference")) != selection["clock_reference"]:
        errors.append("REFERENCE_READBACK_MISMATCH")
    if str(live.get("sysref_policy")) != selection["sysref_policy"]:
...
def _mark_clock_production(
    ...
    *,
    ...
    state = _default_clock_diagnostic_state()
    if selection is not None:
        state.update(
            {
                "profile_id": selection["profile_id"],
                "clock_reference": selection["clock_reference"],
                "sysref_policy": selection["sysref_policy"],
                "sample_rate_msps": sample_rate_msps,
                "center_mhz": center_mhz,
            }
        )
    if live is not None:
        state["profile_sha256"] = live.get("profile_sha256")
    return _persist_clock_diagnostic_state(state)
...
def _recover_clock_diagnostic_failure(
    ...
    *,
    ...
    try:
        production_clock = _production_clock_selection(_body(saved_request))
        result = _apply_clock_profile(
            controller,
            saved_request,
            profile_id=production_clock["profile_id"],
            sample_rate_msps=sample_rate_msps,
            center_mhz=center_mhz,
        )
        live_clock = dict(result["clock"])
        readback_errors = _production_clock_readback_errors(
            production_clock, live_clock
        )
        if readback_errors:
            raise RuntimeError(
                "production clock recovery readback failed: "
                f"errors={readback_errors}; live={live_clock}"
            )
        _mark_ocb1_dynamic()
...
def _clock_diagnostic_restore(request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        live = dict(result["clock"])
        readback_errors = _production_clock_readback_errors(
            production_clock, live
        )
        if readback_errors:
            raise RuntimeError(
                "production clock restore readback failed: "
                f"errors={readback_errors}; live={live}"
            )
...
def _require_external_clock_for_scheduled_sync(
    controller: FEngineController,
) -> None:
    status = controller.require_core().read_status()
    if int(status.get("configured_clock_ref", -1)) != 0:
        raise HelperError(
            "SCHEDULED_SYNC_REQUIRES_EXTERNAL_REFERENCE",
            "scheduled PPS start requires clock_reference=external_10mhz; "
            "onboard_tcxo supports immediate free-run start only",
            exit_code=EXIT_STATE_CONFLICT,
            details={
                "configured_clock_ref": status.get("configured_clock_ref"),
                "configured_sync_mode": status.get("configured_sync_mode"),
            },
        )
...
def _sync_prepare(request: dict[str, Any]) -> dict[str, Any]:
    ...
    output_load = _require_output_load_start_authorization(controller, body)
    rfdc_power = _require_rfdc_power_start_authorization(controller, body)
    _require_external_clock_for_scheduled_sync(controller)
    core = controller.require_core()
