...
Stage 36与Stage 35共有的复可见度图例问题已修复。每个“ADC对＋频点”的幅度、可靠相位和
灰色弱相关相位使用同一Plotly图例组，点击幅度图例会同步切换下方相位图。真实Chrome在
ADC0–ADC1、128.593750 MHz上确认4096点幅度和4096点灰色相位同步隐藏、同步恢复；该频点
没有达到门限的可靠相位点，因此对应空trace保持可见状态但不产生图形。

通用浏览器门禁会实际点击图例并检查所有非空幅相trace。8036上线后服务active、零异常
重启，8035保持健康；证据目录的80个文件已逐项通过SHA-256复核：

```text
/var/lib/t510/measurements/stage36-science-20260906-1852-legend-group-fix-20260906-140535/
```
