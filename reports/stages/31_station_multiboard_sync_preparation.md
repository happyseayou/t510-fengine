...
2026-07-17 已在 attach 的 Vivado 2022.2 GUI 会话中完成 production 配置的 OOC IP、顶层综合、实现、布线和 `write_bitstream`。构建当时只生成和导出本地产物；2026-07-18 已继续发布到单块 T510 并完成 fresh download 和单板 smoke，板端结果见 `../board/stage31_single_board_agent_sync_smoke_20260718.md`。
...
- route status 为 fully routed `229736/229736`，routing errors 为 0。
- setup 为 `WNS=+0.049 ns`、`TNS=0`、失败端点 `0/334996`；hold 为 `WHS=+0.010 ns`、`THS=0`、失败端点 `0/334996`；pulse width 为 `WPWS=+0.052 ns`、`TPWS=0`、失败端点 0。Vivado 结论为所有用户时序约束满足。
- 资源占用为 CLB LUT `103406/425280 (24.31%)`、register `126116/850560 (14.83%)`、BRAM tile `520/1080 (48.15%)`、DSP `600/4272 (14.04%)`、URAM `0/80`。
...
- bit 头部可读到 design `t510_fengine_board_top`、part `xczu47dr-ffve1156-2-i`、Vivado `2022.2`、构建时间 `2026-07-17 23:16:27`。Vivado MCP 的离线 bit 解析器因文件 `32.8 MB` 超过其 `10 MB` 保护上限而不能直接解析，但头部字符串、Vivado run 产物路径和 SHA256 已三方核对。
- run bit 与本地 overlay bit 的 SHA256 均为 `3696845d30dc471f572904b7039aa231bc766a21be05de7796d53704f8d08eec`。overlay 的 `.bit/.hwh/.tcl/manifest` 已重新导出；manifest 仍明确 `ltx=`，本轮没有 ILA probe 文件。
...
板端已更新为 Agent `0.2.0` 和 catalog `fengine-0x00010031`。真实 PREPARE/ARM 在 local PPS `199` 精确提交，最终 state 为 STREAMING，TIME/SPEC 首 sample0 均为 `32788`，同步错误和数据面 drop/route error 为 0。八路 `60.010 MHz`、约 6% DAC 自环在两次 preview 中全部命中，SNR 约 `50.7..52.6 dB` 且无 clipping。
