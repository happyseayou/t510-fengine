...
`TIME_ONLY 900秒平均数字功率`图已加入右侧Y轴的板上PL温度曲线。功率仍完整覆盖900秒，
默认每点100 ms；温度取自同一正式窗口的AMS遥测并按采集实际起点对齐，保留885个约1 Hz
原始点，没有插值或外推。现有遥测证据覆盖0.893–884.851 s，最后约15秒没有温度记录，图中
保持该缺口。记录均值范围为39.231–41.018 °C；图注明确这是板上PL传感器温度，并非噪声
温度或数字功率定标结果。

轻量current-only更新没有重新采集或重算TIME长数据。候选API验证了9000个100 ms功率点和
885个温度点，真实Chromium渲染、线上8035、Stage 36的8036及86个证据文件SHA-256自检均
通过。更新证据位于：
