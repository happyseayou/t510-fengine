...
test -x "${SOURCE}/host_stage29_rx_tune.sh"
...
chmod -R a-w "${RELEASE}"
chmod 0555 "${RELEASE}/t510_time_rx" "${RELEASE}/host_stage29_rx_tune.sh"
ln -sfn "${RELEASE}" /opt/t510-time-rx/current

install -o root -g root -m 0644 \
  "${RELEASE}/90-t510-rx.conf" \
  /etc/sysctl.d/90-t510-rx.conf
install -o root -g root -m 0644 \
  "${RELEASE}/t510-rx-tune.service" \
  /etc/systemd/system/t510-rx-tune.service
install -o root -g root -m 0644 \
  "${RELEASE}/t510-time-rx.service" \
  /etc/systemd/system/t510-time-rx.service
...
echo "Installed Stage 32 receiver release ${RELEASE_ID}"
