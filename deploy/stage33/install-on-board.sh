...
if entry["core_version"] != "0x00010033":
    raise SystemExit("Stage 33 catalog has the wrong core version")
if entry["sha256"] == "0" * 64 or len(entry["sha256"]) != 64:
    raise SystemExit("Stage 33 catalog has not been finalized")
if min(entry["mts_adc_target_latency"], entry["mts_dac_target_latency"]) < 0:
    raise SystemExit("Stage 33 MTS targets have not been finalized")
if entry["mts_adc_target_latency"] == 230 or entry["mts_dac_target_latency"] == 336:
    raise SystemExit("Stage 33 catalog reuses a forbidden retired MTS target")
campaign = entry.get("mts_campaign")
if not isinstance(campaign, dict):
    raise SystemExit("Stage 33 MTS campaign proof is missing")
...
if campaign.get("discovery") != expected or campaign.get("fixed") != expected:
    raise SystemExit("Stage 33 MTS campaigns are not the frozen 40/40 matrices")
if campaign.get("adc_margin") != 20 or campaign.get("dac_margin") != 16:
    raise SystemExit("Stage 33 MTS margins are invalid")
if entry["mts_adc_target_latency"] != campaign.get("observed_adc_max", -1) + 20:
    raise SystemExit("Stage 33 ADC MTS target is not discovery max +20")
if entry["mts_dac_target_latency"] != campaign.get("observed_dac_max", -1) + 16:
    raise SystemExit("Stage 33 DAC MTS target is not discovery max +16")
print(entry["sha256"])
PY
)"
...
echo "Installed Stage 33 Agent/watchdog release ${RELEASE_ID}; FPGA download remains a separate configure action"
