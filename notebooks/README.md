...
该 notebook 文件名沿用 Stage 27h，但当前内容已作为 Stage 27j `TIME_SPEC 100MHz + 4-tap PFB` 的生产控制与生产预览界面，覆盖：
...
Stage 27j 生产合约使用 TIME 端口 `4300..4307`、SPEC 端口 `4308..4323`，SPEC 产品为 `FENGINE_IQ16` 4-tap PFB：`4096` 个 channel、`16` 个 block、`256` 个 channel/block、`1` 个 spectrum-time、`8` 路输入、`8192B` 载荷。版本为 `CORE_VERSION=0x0001002C`，保留 Stage 27i `100MHz` anti-alias halfband FIR，不改变 UDP 合约、24-flow 接收或 `63Gbps+` 生产门槛。由于当前 XFFT IP 没有运行时 reset，notebook 的 `Init + Start` 和 `Apply` 都会先 fresh-download bitstream，再加载 PFB coefficient bank 并启动数据流。
...
旧 notebook 只作为时钟、RFDC、UDP dry-run、DAC/ADC 环回和历史 debug 检查的 bring-up 参考，不作为 Stage 27j 生产验收条件。
