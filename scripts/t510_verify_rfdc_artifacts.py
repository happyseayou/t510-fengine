#!/usr/bin/env python3
"""Verify generated current T510 release RFDC XCI/HWH properties before release."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="current T510 release generated RFDC artifact gate")
    ...
    for slice_name in CONVERTER_SLICES:
        name = f"ADC_CalOpt_Mode{slice_name}"
        _require_equal(errors, "XCI", xci, name, "1")
        if "1" not in hwh_values.get(name, []):
            errors.append(
                f"HWH {name}: expected one value 1, read {hwh_values.get(name)}"
            )
        for name, expected in (
    ...
    result = {
        "ok": not errors,
        "classification": (
            "T510_RFDC_ARTIFACTS_PASS" if not errors else "T510_RFDC_ARTIFACTS_FAIL"
        ),
        "xci": str(xci_path),
        "hwh": str(hwh_path),
