...
cat <<EOF
Published Stage 27c T510 F-engine assets to:
  ${PYNQ_TARGET}:${PYNQ_BRINGUP_DIR}
  ${PYNQ_TARGET}:${PYNQ_JUPYTER_DIR}
...
Board bring-up entry:
  ...
  sudo -E /usr/local/share/pynq-venv/bin/python3 scripts/pynq_stage27c_time_multiflow_bringup.py --no-download --bandwidth-mhz all
EOF
