#!/usr/bin/env python3
"""Finalize the Stage 33 Agent catalog from completed MTS campaign evidence."""
...
def _validate_campaign(value: dict[str, Any], *, phase: str) -> None:
    ...
    if phase == "fixed":
        signatures: dict[str, list[tuple[tuple[int, ...], tuple[int, ...]]]] = {
            "adc": [],
            "dac": [],
        }
        for row in cycles:
            ...
            for kind in signatures:
                ...
                signatures[kind].append((latency, offset))
        for kind, rows in signatures.items():
            if len(rows) != 40 or any(row != rows[0] for row in rows[1:]):
                raise ValueError(
                    f"fixed report {kind.upper()} latency/offset is not repeatable across 40 cycles"
                )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Write Stage 33 bitstream SHA and proven MTS targets into the Agent catalog"
    )
    parser.add_argument(
        "--bitstream",
        default=str(root / "build" / "stage33-vivado" / "latest" / "overlay" / "t510_fengine.bit"),
    )
    parser.add_argument("--discovery-json", required=True)
    parser.add_argument("--fixed-json", required=True)
    parser.add_argument(
        "--catalog",
        default=str(root / "config" / "stage33" / "config.example.json"),
    )
    ...
    if adc_target == 230 or dac_target == 336:
        raise ValueError("Stage 33 must not reuse the retired ADC=230 or DAC=336 target")
    ...
    if entry.get("core_version") != EXPECTED_CORE_VERSION:
        raise ValueError("Stage 33 catalog entry has the wrong core_version")
