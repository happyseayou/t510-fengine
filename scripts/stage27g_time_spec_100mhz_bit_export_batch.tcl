...
# Stage 27g keeps the Stage 27f production wire contract and rebuilds the
...
puts "STAGE27G_TIME_SPEC_100MHZ: USED_LICENSE_KEYS_AFTER_RECREATE=$used_license"
if {![regexp {cmac_usplus@2020\.05 bought} $used_license]} {
    error "Fresh CMAC IP did not bind cmac_usplus to bought license."
}

report_environment -file [file join $report_dir stage27g_time_spec_100mhz_environment.rpt]
report_ip_status -file [file join $report_dir stage27g_time_spec_100mhz_ip_status.rpt]

set cmac_run [get_runs -quiet t510_cmac_usplus_0_synth_1]
if {[llength $cmac_run] != 0} {
    puts "STAGE27G_TIME_SPEC_100MHZ: rebuild CMAC OOC"
    ...
    puts "STAGE27G_TIME_SPEC_100MHZ: CMAC_OOC_STATUS=$cmac_status"
    if {![string match "*Complete*" $cmac_status]} {
        error "CMAC OOC synthesis did not complete."
    }
} else {
    puts "STAGE27G_TIME_SPEC_100MHZ: no CMAC OOC run found"
}
...
if {[llength $xfft_run] == 0 && [llength $xfft_ip] != 0} {
    if {[file exists $xfft_disk_dcp] && [file exists $xfft_disk_done]} {
        puts "STAGE27G_TIME_SPEC_100MHZ: reuse disk F-engine XFFT OOC DCP=$xfft_disk_dcp"
    } else {
        puts "STAGE27G_TIME_SPEC_100MHZ: create missing F-engine XFFT OOC run"
...
if {[llength $xfft_run] != 0} {
    ...
    puts "STAGE27G_TIME_SPEC_100MHZ: rebuild F-engine XFFT OOC for nonrealtime throttle"
    ...
    puts "STAGE27G_TIME_SPEC_100MHZ: XFFT_OOC_STATUS=$xfft_status"
    ...
    puts "STAGE27G_TIME_SPEC_100MHZ: XFFT_OOC_STATUS=disk DCP reused"
...
puts "STAGE27G_TIME_SPEC_100MHZ: rebuild top synthesis"
reset_run synth_1
launch_runs synth_1 -jobs 8
wait_on_run synth_1
set synth_status [get_property STATUS [get_runs synth_1]]
puts "STAGE27G_TIME_SPEC_100MHZ: SYNTH_STATUS=$synth_status"
if {![string match "*Complete*" $synth_status]} {
    error "Top synthesis did not complete."
}
open_run synth_1
report_utilization -file [file join $report_dir stage27g_time_spec_100mhz_synth_utilization.rpt]
report_timing_summary -file [file join $report_dir stage27g_time_spec_100mhz_synth_timing_summary.rpt]

puts "STAGE27G_TIME_SPEC_100MHZ: rebuild implementation through write_bitstream"
reset_run impl_1
launch_runs impl_1 -to_step write_bitstream -jobs 8
wait_on_run impl_1
set impl_status [get_property STATUS [get_runs impl_1]]
puts "STAGE27G_TIME_SPEC_100MHZ: IMPL_STATUS=$impl_status"
if {![string match "*Complete*" $impl_status]} {
    error "Implementation/bitstream did not complete."
}

open_run impl_1
report_route_status -file [file join $report_dir stage27g_time_spec_100mhz_route_status.rpt]
report_utilization -file [file join $report_dir stage27g_time_spec_100mhz_impl_utilization.rpt]
report_bus_skew -warn_on_violation -file [file join $report_dir stage27g_time_spec_100mhz_bus_skew.rpt]
report_timing_summary -file [file join $report_dir stage27g_time_spec_100mhz_impl_timing_summary.rpt]
report_timing -max_paths 20 -file [file join $report_dir stage27g_time_spec_100mhz_worst_paths.rpt]
...
if {$wns ne "" && $wns < 0} {
    set timing_ok 0
    puts "STAGE27G_TIME_SPEC_100MHZ: NEGATIVE_WNS=$wns"
}
if {$whs ne "" && $whs < 0} {
    set timing_ok 0
    puts "STAGE27G_TIME_SPEC_100MHZ: NEGATIVE_WHS=$whs"
}
if {!$timing_ok} {
    error "Stage 27g timing is negative; refusing to export overlay."
}

source [file join $repo_root scripts export_overlay.tcl]
puts "STAGE27G_TIME_SPEC_100MHZ: overlay export complete"
