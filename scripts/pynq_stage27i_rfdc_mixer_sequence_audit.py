...
def main() -> int:
    ...
    if "--stage27i-rfdc-mixer-sequence-audit" not in sys.argv:
        sys.argv.insert(1, "--stage27i-rfdc-mixer-sequence-audit")
    ...
    if "--no-download-each-case" not in sys.argv and "--download-each-case" not in sys.argv:
        sys.argv.append("--no-download-each-case")
    if "--output" not in sys.argv:
        date_tag = time.strftime("%Y%m%d")
        sys.argv.extend(
            [
                "--output",
                str(root / "reports" / "board" / f"stage27i_rfdc_mixer_sequence_audit_external_pps_{date_tag}.json"),
            ]
        )
