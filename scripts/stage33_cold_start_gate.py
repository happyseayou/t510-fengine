#!/usr/bin/env python3
"""Verify a freshly cold-booted Stage 33 board, services, configure, and resume."""
...
def main() -> int:
    ...
    output = (root / args.output_dir).resolve() / f"stage33_cold_start_{args.tag}.json"
    if output.exists():
    ...
    evidence: dict[str, Any] = {
        "ok": False,
        "classification": "STAGE33_COLD_START_IN_PROGRESS",
        "stage": 33,
        "tag": args.tag,
    ...
    if args.dry_run:
        evidence.update(
            {
                "ok": True,
                "classification": "STAGE33_COLD_START_DRY_RUN",
    ...
    try:
        ...
        idle_health = _stage33_rfdc_health(idle, require_valid=False)
        evidence["idle"] = idle
        ...
        for cycle in range(2):
            ...
            health = _stage33_rfdc_health(streaming, require_valid=True)
            row = {
                "cycle": cycle,
                "streaming": streaming,
                "rfdc_health": health,
        ...
        evidence["classification"] = (
            "STAGE33_COLD_START_PASS"
            if evidence["ok"]
            else "STAGE33_COLD_START_FAIL"
        )
