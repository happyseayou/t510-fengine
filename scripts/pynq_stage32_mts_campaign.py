#!/usr/bin/env python3
"""Run the Stage 32 RFDC MTS discovery or fixed-latency campaign on T510.
...
def _assess_cycle(
    ...
    *,
    ...
    if str(clock.get("profile_id")) != "stage32_160_10m_cont_manual_clkin2":
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Stage 32 RFDC MTS campaign")
    ...
    output = (
        Path(args.output)
        if args.output
        else _root() / "reports" / "board" / f"stage32c_mts_{args.phase}.json"
    )
    actions = _campaign_actions(args)
    result: dict[str, Any] = {
        "classification": f"STAGE32C_MTS_{args.phase.upper()}_IN_PROGRESS",
        "ok": False,
        "stage": "32c",
        "phase": args.phase,
    ...
    result["classification"] = (
        f"STAGE32C_MTS_{args.phase.upper()}_{'PASS' if result['ok'] else 'FAIL'}"
    )
    _write_checkpoint(output, result)
