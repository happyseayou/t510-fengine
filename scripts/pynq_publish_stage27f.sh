...
rsync -av \
  "${REPO_ROOT}/scripts/pynq_stage27f_science_fengine_bringup.py" \
  "${PYNQ_TARGET}:${PYNQ_BRINGUP_DIR}/scripts/"

rsync -av \
  "${REPO_ROOT}/notebooks/13_astronomer_rf_observation_console.ipynb" \
...
cat <<EOF
Published Stage 27f T510 F-engine assets to:
  ${PYNQ_TARGET}:${PYNQ_BRINGUP_DIR}
  ${PYNQ_TARGET}:${PYNQ_JUPYTER_DIR}
...
Board bring-up entry:
  ...
  sudo -E /usr/local/share/pynq-venv/bin/python3 scripts/pynq_stage27f_science_fengine_bringup.py --no-download --matrix smoke

Jupyter production entry:
  ${PYNQ_JUPYTER_DIR}/notebooks/13_astronomer_rf_observation_console.ipynb

Stage 27f production scope is intentionally narrow: TIME/SPEC science streams
