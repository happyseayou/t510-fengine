#!/usr/bin/env python3
"""Two-phase scheduled-sync coordinator for a small station without a Center Hub.
...
def read_snapshots(boards: list[tuple[str, int]]) -> list[tuple[str, int, dict]]:
    def read_one(board: tuple[str, int]) -> tuple[str, int, dict]:
        url, board_id = board
        result = request(url, "/api/v2/sync/status")
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if bool(dict(result.get("sync", {})).get("error", False)):
            raise RuntimeError(f"board {board_id} scheduled-sync error: {result['sync']}")
        if healthy:
    ...
    raise RuntimeError(
        f"board {board_id} did not establish sustained scheduled data flow: {last}"
    )
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=len(snapshots)) as pool:
            for url, board_id, sync in snapshots:
                ...
                prepare_jobs.append((pool.submit(request, url, "/api/v2/sync/prepare", body), url, board_id))
        ...
        with ThreadPoolExecutor(max_workers=len(prepared)) as pool:
            for url, board_id in prepared:
                future = pool.submit(
                    request, url, "/api/v2/sync/arm", {"expected_board_id": board_id}
                )
        ...
        for url, board_id in prepared:
            try:
                request(url, "/api/v2/sync/abort", {"expected_board_id": board_id})
