...
MODE="${1:---build-only}"
RELEASE_ID="${STAGE30_RELEASE_ID:-$(git -C "${ROOT}" rev-parse --short=12 HEAD)-$(date -u +%Y%m%d%H%M%S)}"
BUILD_DIR="${ROOT}/build/stage30"
STAGE="${BUILD_DIR}/${RELEASE_ID}"
REMOTE_STAGE="/home/xilinx/.cache/t510-stage30/${RELEASE_ID}"
EXPECTED_SHA="7486a55b6f7e50e5875474e7d85299b107e9384cfff454316f64f2d3d7e9800d"

case "${MODE}" in
  --build-only|--install) ;;
  *) echo "usage: pynq_publish_stage30.sh [--build-only|--install]" >&2; exit 2 ;;
esac
...
install -m 0644 "${ROOT}/config/stage30/config.example.json" "${STAGE}/config/config.example.json"
install -m 0644 "${ROOT}/deploy/stage30/t510-agent.service" "${STAGE}/deploy/t510-agent.service"
install -m 0755 "${ROOT}/deploy/stage30/install-on-board.sh" "${STAGE}/deploy/install-on-board.sh"
...
if readelf -l "${STAGE}/bin/t510-board-agent" | grep -q 'Requesting program interpreter'; then
  echo "Stage 30 binary is dynamically linked; expected static musl" >&2
  exit 1
fi

if [[ "${MODE}" == "--build-only" ]]; then
  echo "Built static Stage 30 release at ${STAGE}"
  exit 0
fi
...
curl --fail --silent --show-error --max-time 10 \
  "http://${TARGET#*@}:8010/api/v1/status" >/dev/null
echo "Installed ${RELEASE_ID}; only read-only Agent endpoints were called"
