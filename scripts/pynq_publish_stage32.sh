...
MODE="${1:---build-only}"
RELEASE_ID="${STAGE32_RELEASE_ID:-stage32-$(git -C "${ROOT}" rev-parse --short=12 HEAD)-$(date -u +%Y%m%d%H%M%S)}"
BUILD_DIR="${ROOT}/build/stage32"
STAGE="${BUILD_DIR}/${RELEASE_ID}"
REMOTE_STAGE="/home/xilinx/.cache/t510-stage32/${RELEASE_ID}"
EXPECTED_SHA="47117c9e656cfd8345125ef0130eb91a5ec0868cef59931b40b957da29f31234"

case "${MODE}" in
  --build-only|--install) ;;
  *) echo "usage: pynq_publish_stage32.sh [--build-only|--install]" >&2; exit 2 ;;
esac
...
install -m 0755 \
  "${ROOT}/python/t510_ref_watchdog.py" \
  "${STAGE}/python/t510_ref_watchdog.py"
...
install -m 0644 "${ROOT}/config/stage32/config.example.json" "${STAGE}/config/config.example.json"
install -m 0644 "${ROOT}/deploy/stage30/t510-agent.service" "${STAGE}/deploy/t510-agent.service"
install -m 0644 \
  "${ROOT}/deploy/stage32/t510-ref-watchdog.service" \
  "${STAGE}/deploy/t510-ref-watchdog.service"
install -m 0644 \
  "${ROOT}/deploy/stage32/t510-agent.service.d/center-hub.conf" \
  "${STAGE}/deploy/t510-agent.service.d/center-hub.conf"
install -m 0755 "${ROOT}/deploy/stage32/install-on-board.sh" "${STAGE}/deploy/install-on-board.sh"
...
if readelf -l "${STAGE}/bin/t510-board-agent" | grep -q 'Requesting program interpreter'; then
  echo "Stage 32 binary is dynamically linked; expected static musl" >&2
  exit 1
fi

if [[ "${MODE}" == "--build-only" ]]; then
  echo "Built static Stage 32 release at ${STAGE}"
  exit 0
fi
...
echo "Installed ${RELEASE_ID}; run configure to fresh-download the Stage 32 bitstream"
