...
def fixed_latency_target(max_observed: int, *, kind: str) -> dict[str, int]:
    """Choose an RFDC fixed target strictly above the quantized margin floor.

    RFDC latency is quantized in factor-12 steps.  Merely adding the nominal
    margin can round onto the driver's minimum-possible boundary, which the
    RFDC MTS implementation rejects.  Preserve the registered +20/+16 margin,
    round that floor upward, then reserve one complete quantum of headroom.
    """
