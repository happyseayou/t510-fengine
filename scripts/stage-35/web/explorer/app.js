...
async function renderTimeSingle(adcs, guard = () => true) {
  ...
  figure = createFigure(card, `single-time-long-power-page-${state.page}`,
    `TIME_ONLY 900秒平均数字功率（每点 ${firstLong.cadence_ms} ms）`, {
      ...
      source: sourceText(firstLong.source) + (temperature ? ` PL温度来自同一正式窗口的板上AMS遥测，共${temperature.points}个原始约1秒采样点，覆盖至${number(temperature.coverage_seconds, 3)}秒；按实际时间戳对齐，未插值或外推。` : ""), formula: firstLong.formula,
...
async function init() {
  ...
  set("timeCapture", "time_capture", params.get("time") || "A-pre");
  ...
  set("fftCapture", "fft_capture", params.get("time") || "A-pre");
