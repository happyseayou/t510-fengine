# Prepare the current T510 implementation in the already-open demo-ant project.
...
if {[current_project -quiet] eq ""} {
    error "current T510 release requires the existing demo-ant project to be open"
}
...
if {$project_name ne "demo-ant" || $project_dir ne $repo_root} {
    error "current T510 release must update demo-ant at $repo_root; open project is $project_name at $project_dir"
}
if {$project_part ne "xczu47dr-ffve1156-2-i"} {
    error "current T510 release project part mismatch: expected xczu47dr-ffve1156-2-i, read $project_part"
}
foreach run_name {synth_1 impl_1} {
    set run [get_runs -quiet $run_name]
    if {[llength $run] != 1} {
        error "current T510 release requires existing run $run_name"
    }
...
if {[llength $source_defines] != 0} {
    error "current T510 release current RTL must not depend on build defines; found $source_defines"
}
...
if {$sim_defines ne "T510_SIM_FFT_MODEL" && $sim_defines ne {T510_SIM_FFT_MODEL}} {
    error "current T510 release simulation must use only T510_SIM_FFT_MODEL; found $sim_defines"
}
set bd_file [get_files -quiet -all */t510_rfdc_bd.bd]
if {[llength $bd_file] != 1} {
    error "current T510 release expected exactly one t510_rfdc_bd.bd, found $bd_file"
}
...
puts "T510_CURRENT_PROJECT_PREPARED project=$project_name directory=$project_dir part=$project_part"
puts "T510_CURRENT_PROJECT_DEFINES source=$source_defines sim=$sim_defines"
