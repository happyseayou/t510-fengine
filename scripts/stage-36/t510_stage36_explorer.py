#!/usr/bin/env python3
"""Read-only Stage 36 explorer with only TIME, F-engine, and Allan views."""
...
DATA_ROOT = Path("/var/lib/t510").resolve()
...
def fixed_path(value: str | Path, *, file: bool = False) -> Path:
    ...
    if path != DATA_ROOT and DATA_ROOT not in path.parents:
        raise ValueError(f"path escapes the fixed T510 data root: {path}")
...
class SimpleData:
    def __init__(self, config_path: Path, helper_dir: Path):
        ...
        if self.config.get("format") not in {
            ...
            "T510_STAGE36_SIMPLE_EXPLORER_CONFIG_V1",
        }:
        ...
        if self.center_mhz != 200.0:
            raise ValueError("the Stage 36 report is frozen to the 200 MHz center")
        ...
        if self.raw_manifest.get("format") not in {"T510_STAGE35_SIMPLE_RAW_INDEX_V1", "T510_STAGE36_SIMPLE_RAW_INDEX_V1"}:
        ...
        if (
            self.time_long_index.get("format") != "T510_TIME_CAPTURE_LONG_ARRAYS_V1"
    ...
    def meta(self) -> dict[str, Any]:
        ...
        return {
            "format": "T510_STAGE36_SIMPLE_EXPLORER_META_V1",
            "title": "Stage 36：TIME_ONLY、F-engine 与 Allan 方差",
            ...
            "defaults": {
                "adcs": [0], "pairs": [[0, 1]],
                "bins": [3134, 3182, 3328], "time_capture": next(iter(self.time_records)),
            ...
            "stage35_comparison": self.stage35_comparison,
            "technical_sources": {
    ...
    def _capture(self, query: dict[str, list[str]]) -> str:
        capture = query_one(query, "capture", next(iter(self.time_records)))
    ...
    def allan(self, query: dict[str, list[str]]) -> dict[str, Any]:
        ...
        return {
            "format": "T510_STAGE36_SIMPLE_ALLAN_V1", "subject": subject,
...
class Handler(BaseHTTPRequestHandler):
    server_version = "T510Stage36SimpleExplorer/1"
    ...
    def log_message(self, fmt: str, *args: Any) -> None:
        sys.stderr.write("stage36-simple-explorer: " + fmt % args + "\n")
    ...
    def route(self, *, head: bool) -> None:
        ...
        try:
            if parsed.path == "/healthz":
                self.send_json({"ok": True, "application": "stage36-simple"}, head=head)
...
def main() -> int:
    ...
    parser.add_argument("--bind", default="0.0.0.0:8036")
