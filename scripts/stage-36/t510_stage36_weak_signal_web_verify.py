...
out=Path('/home/astrolab/.cache/t510/weak-signal-web-check-20260912');out.mkdir(exist_ok=True)
...
try:
 ...
 request('POST',f'{d}/session/{s}/url',{'url':'http://127.0.0.1:8036/static/weak-signal.html'})
 wait_until(lambda:run('return window.weakReady===true'))
 assert run("return document.querySelectorAll('#pair option').length===28 && document.getElementById('amp').data.length===6")
 native=json.load(urllib.request.urlopen('http://127.0.0.1:8036/static/weak-signal-data/trends/before-0-2-3201.json'))
 ...
 wait_until(lambda:run("return window.weakReady && document.getElementById('amp').data.length===4"))
 ...
 wait_until(lambda:run("return window.weakReady&&['amp','phase'].every(id=>document.getElementById(id).data[0].visible==='legendonly')"))
 run("document.getElementById('pair').value='5-7';document.getElementById('pair').dispatchEvent(new Event('change'))")
 wait_until(lambda:run("return window.weakReady&&document.getElementById('status').textContent.includes('ADC5–ADC7')"))
 ...
 (out/'verification.json').write_text(json.dumps({'status':'PASS','checks':['28pairs','native1200 corrected amplitude/phase values','OFF selection','real legend click sync','pair switch','mobile no overflow']}))
 print('PASS')
finally:
