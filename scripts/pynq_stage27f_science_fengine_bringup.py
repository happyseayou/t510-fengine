...
EXPECTED_CORE_VERSION = 0x0001_0021
...
def _parse_cases(value: str) -> list[tuple[str, int]]:
    ...
    if key == "smoke":
        return [("spec_only", 100), ("time_spec", 20), ("time_spec", 100)]
    if key == "full":
        return [
            ("time_only", 20),
            ("time_only", 100),
...
def _validate_reject(core: Any) -> dict[str, Any]:
    try:
        core.configure_science_27f(bandwidth_mhz=200, output_mode="time_spec", start=False)
    except ValueError as exc:
        return {
            "mode": "TIME_SPEC",
            "bandwidth_mhz": 200,
            "classification": "STAGE27F_TIME_SPEC_200_REJECT_PASS",
            "ok": True,
            "error": str(exc),
        }
        ...
        return {
            "mode": "TIME_SPEC",
            "bandwidth_mhz": 200,
            "classification": "STAGE27F_TIME_SPEC_200_REJECT_FAIL",
            "ok": False,
    ...
    return {
        "mode": "TIME_SPEC",
        "bandwidth_mhz": 200,
        "classification": "STAGE27F_TIME_SPEC_200_NOT_REJECTED",
        "ok": False,
        "error": "configure_science_27f accepted TIME_SPEC at 200MHz",
    }
...
def main() -> int:
    ...
    default_output = _repo_root() / "reports" / "board" / "stage27f_science_fengine_board.json"
    parser = argparse.ArgumentParser(description="Stage 27f full F-engine IQ16 science board validation on PYNQ.")
    parser.add_argument("--bitfile", default=str(default_bitfile))
    parser.add_argument("--no-download", action="store_true")
    parser.add_argument("--seconds", type=float, default=2.0)
    parser.add_argument("--matrix", default="smoke", help="smoke, full, or comma list like spec_only:100,time_spec:20")
    ...
    parser.add_argument("--spec-route-count", type=int, default=64)
    ...
    parser.add_argument("--pfb-taps", type=int, default=4)
    ...
    for index, (mode, bandwidth_mhz) in enumerate(_parse_cases(args.matrix)):
        validations.append(
            core.run_stage27f_science_validation(
                configure=True,
    ...
    classification = "STAGE27F_FULL_SCIENCE_PASS" if ok else "STAGE27F_FULL_SCIENCE_FAIL"
    result = {
        ...
        "production_scope": {
            ...
            "legacy_debug_policy": (
                "dry-run, raw witness, debug FFT, and legacy 64b SPEC paths are bring-up aids only; "
                "they are not Stage 27f production pass criteria"
            ),
        },
        "science_data_scope": "Stage 27f production TIME/SPEC science streams plus Jupyter control/preview",
        "payload_contract": "SPEC product_id=FENGINE_IQ16 nchan=4096 64 blocks x 64 channels x 4 spectra-time x 8 inputs x IQ16",
