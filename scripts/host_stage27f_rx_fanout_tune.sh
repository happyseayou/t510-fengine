...
usage() {
  cat <<'EOF'
Usage: scripts/host_stage27f_rx_fanout_tune.sh [--no-set] [--port-base N] [--port-count N] [--queue-base N] [--queue-count N] [interface]

Stage 27f host-side PACKET_FANOUT / ntuple helper for full TIME + 4096-bin SPEC.
...
Optional:
  STAGE27F_SET_RSS_HASH=1  attempt ethtool -N <iface> rx-flow-hash udp4 sdfn
  STAGE27F_SET_NTUPLE=1    steer dst ports 4300..4371 to RX queues, modulo available queue count
  STAGE27F_CLEAR_NTUPLE=1  delete ntuple rules loc queue_base..queue_base+port_count-1
EOF
}
...
section "Stage 27f RX Fanout Context"
...
if [[ "${STAGE27F_SET_RSS_HASH:-0}" == "1" ]]; then
...
if [[ "${STAGE27F_CLEAR_NTUPLE:-0}" == "1" ]]; then
  section "Clear Stage 27f Ntuple Rules"
  if [[ "${EUID}" -ne 0 ]]; then
...
if [[ "${STAGE27F_SET_NTUPLE:-0}" == "1" ]]; then
