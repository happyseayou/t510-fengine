...
def _per_flow_gap_delta(after: dict[str, Any], before: dict[str, Any], name: str) -> int:
    before_by_flow = {
        int(flow.get("flow_id", -1)): int(flow.get(name, 0))
        for flow in before.get("per_flow", [])
        if isinstance(flow, dict)
    }
    total = 0
    for flow in after.get("per_flow", []):
        if not isinstance(flow, dict):
            continue
        flow_id = int(flow.get("flow_id", -1))
        total += int(flow.get(name, 0)) - before_by_flow.get(flow_id, 0)
    return total


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Stage 27d Rust PACKET_FANOUT TIME receiver state.")
    ...
    parser.add_argument("--min-active-workers", type=int, default=4)
    parser.add_argument("--min-display-hz", type=float, default=50.0)
    ...
    default_output = _repo_root() / "reports" / "board" / f"stage27d_rust_rx_{args.bandwidth_mhz}mhz.json"
    ...
    worker_count = int(stats_after.get("worker_count", 0))
    ...
    if worker_count < int(args.min_active_workers):
        errors.append(f"WORKER_COUNT_LOW worker_count={worker_count} required={int(args.min_active_workers)}")
    if active_worker_count < int(args.min_active_workers):
        errors.append(
            f"ACTIVE_WORKERS_LOW active={active_worker_count} required={int(args.min_active_workers)}"
        )
    ...
    if args.require_display and display_hz < float(args.min_display_hz):
        errors.append(f"DISPLAY_UPDATE_LOW {display_hz:.3f} < {float(args.min_display_hz):.3f}")

    if active_worker_count < int(args.min_active_workers) or backend != "fanout":
        classification = "BLOCK_STAGE27D_FANOUT_NOT_DISTRIBUTING"
    elif nic_error_delta or ring_drops_delta or worker_ring_drops_delta or kernel_drops_delta:
        classification = "BLOCK_STAGE27D_NIC_DROP_ERROR"
    elif args.require_display and display_hz < float(args.min_display_hz):
        classification = "BLOCK_STAGE27D_DISPLAY_TAP_LIMIT"
    elif rx_processed_pps < threshold_pps or seq_gaps_delta or frame_gaps_delta or sample0_gaps_delta:
        classification = "BLOCK_STAGE27D_HOST_RX_LIMIT"
    elif per_flow_seq_gaps_delta or per_flow_frame_gaps_delta or per_flow_sample0_gaps_delta:
        classification = "BLOCK_STAGE27D_HOST_RX_LIMIT"
    elif errors:
        classification = "HOST_STAGE27D_RUST_RX_FAIL"
    else:
        classification = "HOST_STAGE27D_RUST_RX_PASS"

    result = {
        "classification": classification,
        "ok": classification == "HOST_STAGE27D_RUST_RX_PASS",
        ...
        "fanout_group": stats_after.get("fanout_group"),
        "fanout_mode": stats_after.get("fanout_mode"),
        ...
        "gap_deltas": {
            ...
            "per_flow_seq_gaps": per_flow_seq_gaps_delta,
            "per_flow_frame_gaps": per_flow_frame_gaps_delta,
            "per_flow_sample0_gaps": per_flow_sample0_gaps_delta,
        },
        "parse_errors_delta": parse_errors_delta,
        "ring_drops_delta": ring_drops_delta,
        "worker_ring_drops_delta": worker_ring_drops_delta,
        ...
        "samples": samples,
        "errors": errors,
        "result": "PASS" if classification == "HOST_STAGE27D_RUST_RX_PASS" else "BLOCKED",
    }
...
if __name__ == "__main__":
    try:
        ...
        print(json.dumps({"classification": "HOST_STAGE27D_RUST_RX_ERROR", "error": f"{type(exc).__name__}: {exc}"}))
        raise
