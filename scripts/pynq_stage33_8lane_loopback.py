#!/usr/bin/env python3
"""Stage 33 eight-lane DAC-to-ADC loopback gate."""
...
def main() -> int:
    ...
    parser.add_argument("--adc-target", type=int, required=True)
    parser.add_argument("--dac-target", type=int, required=True)
    ...
    parser.add_argument(
        "--output",
        default=str(_root() / "reports" / "board" / "stage33_8lane_loopback.json"),
    )
    ...
    if args.adc_target == 230 or args.dac_target == 336:
        parser.error("Stage 33 must not reuse ADC=230 or DAC=336")

    phases = (0.0, 45.0, 90.0, 135.0, 180.0, -135.0, -90.0, -45.0)
    core = T510FEngine(args.bitfile, download=not args.no_download)
    result: dict[str, Any] = {
        "classification": "STAGE33_8LANE_LOOPBACK_IN_PROGRESS",
        "ok": False,
    ...
    try:
        ...
        result.update(
            {
                "classification": (
                    "STAGE33_8LANE_LOOPBACK_PASS"
                    if not errors
                    else "STAGE33_8LANE_LOOPBACK_FAIL"
                ),
                "ok": not errors,
        ...
        result["classification"] = "STAGE33_8LANE_LOOPBACK_FAIL"
        ...
        try:
            ...
            result["dac_muted"] = True
        except Exception as exc:
            result.setdefault("errors", []).append(
                f"DAC_MUTE_FAILED: {type(exc).__name__}: {exc}"
            )
            result["ok"] = False
            result["classification"] = "STAGE33_8LANE_LOOPBACK_FAIL"
