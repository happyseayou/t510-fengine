...
def _stage33_rfdc_health(snapshot: dict[str, Any], *, require_valid: bool) -> dict[str, Any]:
    errors: list[str] = []
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Stage 33 simultaneous board/host UDP gate")
    ...
    parser.add_argument(
        "--remote-output",
        default="/home/astrolab/.cache/t510-stage33/stage33_host_gate.json",
    )
    ...
    evidence: dict[str, Any] = {
        "classification": (
            f"STAGE33_{args.sample_rate_msps}MSPS_{args.mode.upper()}_BOARD_HOST_IN_PROGRESS"
        ),
        "ok": False,
        "stage": 33,
    ...
    try:
        ...
        if str(idle.get("core_version", "")).lower() != "0x00010033":
            raise RuntimeError(f"Stage 33 core mismatch before START: {idle.get('core_version')}")
        ...
        if not idle_rfdc_health["ok"]:
            raise RuntimeError(f"Stage 33 RFDC contract failed before START: {idle_rfdc_health['errors']}")
        ...
        evidence["rfdc_health_before"] = _stage33_rfdc_health(
        ...
        evidence["rfdc_health_after"] = _stage33_rfdc_health(
        ...
        evidence["classification"] = (
            f"STAGE33_{args.sample_rate_msps}MSPS_{args.mode.upper()}_BOARD_HOST_"
        ...
        if started:
            try:
                stop = _result(
                    _http_json(
                        base + "/api/v2/stop",
                        body={"reason": "stage33_agent_host_gate_complete"},
                        timeout=30.0,
                    )
                )
                evidence["stop"] = {
        ...
        if not evidence["ok"]:
            evidence["classification"] = (
                f"STAGE33_{args.sample_rate_msps}MSPS_{args.mode.upper()}_BOARD_HOST_FAIL"
            )
