...
MODE="${1:---build-only}"
RELEASE_ID="${STAGE33_RELEASE_ID:-stage33-$(git -C "${ROOT}" rev-parse --short=12 HEAD)-$(date -u +%Y%m%d%H%M%S)}"
BUILD_DIR="${ROOT}/build/stage33"
STAGE="${BUILD_DIR}/${RELEASE_ID}"
REMOTE_STAGE="/home/xilinx/.cache/t510-stage33/${RELEASE_ID}"
CATALOG="${ROOT}/config/stage33/config.example.json"
OVERLAY_DIR="${STAGE33_OVERLAY_DIR:-${ROOT}/build/stage33-vivado/latest/overlay}"

case "${MODE}" in
  --build-only|--install) ;;
  *) echo "usage: pynq_publish_stage33.sh [--build-only|--install]" >&2; exit 2 ;;
esac
...
if entry.get("core_version") != "0x00010033":
    raise SystemExit("catalog core version is not Stage 33")
if entry.get("sha256") == "0" * 64:
    raise SystemExit("catalog is not finalized; run both MTS campaigns and stage33_finalize_catalog.py")
...
install -m 0644 "${ROOT}/deploy/stage33/t510-agent.service" "${STAGE}/deploy/t510-agent.service"
install -m 0644 "${ROOT}/deploy/stage33/t510-ref-watchdog.service" "${STAGE}/deploy/t510-ref-watchdog.service"
install -m 0644 "${ROOT}/deploy/stage33/t510-agent.service.d/center-hub.conf" "${STAGE}/deploy/t510-agent.service.d/center-hub.conf"
install -m 0755 "${ROOT}/deploy/stage33/install-on-board.sh" "${STAGE}/deploy/install-on-board.sh"
...
if readelf -l "${STAGE}/bin/t510-board-agent" | grep -q 'Requesting program interpreter'; then
  echo "Stage 33 binary is dynamically linked; expected static musl" >&2
  exit 1
fi

if [[ "${MODE}" == "--build-only" ]]; then
  echo "Built static Stage 33 release at ${STAGE}"
  exit 0
fi
...
echo "Installed ${RELEASE_ID}; run configure to fresh-download the Stage 33 bitstream"
