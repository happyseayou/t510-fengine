...
EXPECTED_CORE_VERSION = 0x0001_001B
...
def main() -> int:
    ...
    default_output = _repo_root() / "reports" / "board" / "stage25_time_low_rate_live_bringup.json"
    parser = argparse.ArgumentParser(description="Stage 25 low-rate TIME live CMAC bring-up on PYNQ.")
    ...
    parser.add_argument("--bandwidth-mhz", default="20")
    parser.add_argument("--time-payload-nsamp", type=int, default=64)
    parser.add_argument("--time-live-interval-beats", type=int, default=7680)
    ...
    validation = core.run_stage25_time_live_validation(
        configure=True,
    ...
    result = {
        ...
        "host_pcap_next_step": {
            "script": "scripts/host_stage25_time_pcap_check.py",
            "interface": "ens2f0np0",
            "filter": "udp and port 4300",
            ...
            "expected_sample0_step": 245760,
        },
        "result": "PASS" if not errors and bool(validation.get("ok", False)) else "FAIL",
        "errors": errors,
    }
