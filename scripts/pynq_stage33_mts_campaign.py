#!/usr/bin/env python3
"""Run the Stage 33 RFDC MTS discovery or fixed-latency campaign on T510.
...
def _assess_cycle(
    ...
    *,
    ...
    for kind, target in (("adc", adc_target), ("dac", dac_target)):
        ...
        if not latencies:
            errors.append(f"{kind.upper()}_LATENCY_MISSING")
        if phase == "fixed":
            if int(config.get("target_latency", -1)) != int(target):
                errors.append(f"{kind.upper()}_TARGET_READBACK_MISMATCH")
            if any(value > int(target) for value in latencies):
                errors.append(f"{kind.upper()}_LATENCY_OVER_TARGET")
...
def _fixed_repeatability(cycles: list[dict[str, Any]]) -> dict[str, Any]:
    signatures: dict[str, list[dict[str, list[int]]]] = {"adc": [], "dac": []}
    errors: list[str] = []
    for row in cycles:
        ...
        for kind in ("adc", "dac"):
            ...
            if len(signature["latency"]) != 4 or len(signature["offset"]) != 4:
                errors.append(f"{kind.upper()}_REPEATABILITY_READBACK_INCOMPLETE")
            signatures[kind].append(signature)
    for kind, values in signatures.items():
        if len(values) != 40:
            ...
            errors.append(f"{kind.upper()}_LATENCY_OR_OFFSET_NOT_REPEATABLE")
    return {
        "ok": not errors,
        "cycles": min((len(values) for values in signatures.values()), default=0),
        "baseline": {
            kind: values[0] if values else None for kind, values in signatures.items()
        },
...
def _targets(args: argparse.Namespace) -> tuple[int, int]:
    ...
    if int(adc_target) == 230 or int(dac_target) == 336:
        raise ValueError("Stage 33 fixed phase must not reuse ADC=230 or DAC=336")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Stage 33 RFDC MTS campaign")
    parser.add_argument("--phase", choices=("discovery", "fixed"), required=True)
    parser.add_argument(
        "--bitfile",
        default=str(_root() / "build" / "stage33-vivado" / "latest" / "overlay" / "t510_fengine.bit"),
    )
    ...
    output = (
        Path(args.output)
        if args.output
        else _root() / "reports" / "board" / f"stage33_mts_{args.phase}.json"
    )
    bitfile = Path(args.bitfile).resolve(strict=True)
    actions = _campaign_actions(args)
    result: dict[str, Any] = {
        "classification": f"STAGE33_MTS_{args.phase.upper()}_IN_PROGRESS",
        "ok": False,
        "stage": "33",
        "phase": args.phase,
    ...
    if args.phase == "discovery":
        ...
        result["fixed_repeatability"] = _fixed_repeatability(result["cycles"])
    ...
    result["classification"] = (
        f"STAGE33_MTS_{args.phase.upper()}_{'PASS' if result['ok'] else 'FAIL'}"
    )
    _write_checkpoint(output, result)
