#!/usr/bin/env python3
"""Current T510 eight-lane DAC-to-ADC loopback gate."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--output",
        default=str(
            _root() / "build" / "board" / "latest" / "evidence" / "8lane_loopback.json"
        ),
    )
    ...
    if args.adc_target == 230 or args.dac_target == 336:
        parser.error("current T510 release must not reuse ADC=230 or DAC=336")

    phases = (0.0, 45.0, 90.0, 135.0, 180.0, -135.0, -90.0, -45.0)
    core = T510FEngine(args.bitfile, download=not args.no_download)
    result: dict[str, Any] = {
        "classification": "T510_8LANE_LOOPBACK_IN_PROGRESS",
        "ok": False,
    ...
    try:
        ...
        result.update(
            {
                "classification": (
                    "T510_8LANE_LOOPBACK_PASS"
                    if not errors
                    else "T510_8LANE_LOOPBACK_FAIL"
                ),
                "ok": not errors,
        ...
        result["classification"] = "T510_8LANE_LOOPBACK_FAIL"
