...
# Stage 25 adds RTL files after the Stage 24d project was created. Refresh the
# project filesets before synthesis so batch builds match the XSim source list.
...
puts "STAGE25_TIME_LIVE: USED_LICENSE_KEYS_AFTER_RECREATE=$used_license"
if {![regexp {cmac_usplus@2020\.05 bought} $used_license]} {
    error "Fresh CMAC IP did not bind cmac_usplus to bought license."
}

report_environment -file [file join $report_dir stage25_time_live_environment.rpt]
report_ip_status -file [file join $report_dir stage25_time_live_ip_status.rpt]

set cmac_run [get_runs -quiet t510_cmac_usplus_0_synth_1]
if {[llength $cmac_run] != 0} {
    puts "STAGE25_TIME_LIVE: rebuild CMAC OOC"
    ...
    puts "STAGE25_TIME_LIVE: CMAC_OOC_STATUS=$cmac_status"
    if {![string match "*Complete*" $cmac_status]} {
        error "CMAC OOC synthesis did not complete."
    }
} else {
    puts "STAGE25_TIME_LIVE: no CMAC OOC run found"
}

puts "STAGE25_TIME_LIVE: rebuild top synthesis"
reset_run synth_1
launch_runs synth_1 -jobs 8
wait_on_run synth_1
set synth_status [get_property STATUS [get_runs synth_1]]
puts "STAGE25_TIME_LIVE: SYNTH_STATUS=$synth_status"
if {![string match "*Complete*" $synth_status]} {
    error "Top synthesis did not complete."
}

puts "STAGE25_TIME_LIVE: rebuild implementation through write_bitstream"
reset_run impl_1
launch_runs impl_1 -to_step write_bitstream -jobs 8
wait_on_run impl_1
set impl_status [get_property STATUS [get_runs impl_1]]
puts "STAGE25_TIME_LIVE: IMPL_STATUS=$impl_status"
if {![string match "*Complete*" $impl_status]} {
    error "Implementation/bitstream did not complete."
}

source [file join $repo_root scripts export_overlay.tcl]
puts "STAGE25_TIME_LIVE: overlay export complete"
