...
EXPECTED_CORE_VERSION = 0x0001_0025
...
def _parse_cases(value: str) -> list[tuple[str, int]]:
    ...
    if key in ("converge", "convergence", "stage27g", "time_spec_100"):
...
def _validate_reject(core: Any) -> dict[str, Any]:
    try:
        core.configure_science_27g(bandwidth_mhz=200, output_mode="time_spec", start=False)
    except ValueError as exc:
        return {
            "mode": "TIME_SPEC",
            "bandwidth_mhz": 200,
            "classification": "STAGE27G_TIME_SPEC_200_REJECT_PASS",
            "ok": True,
            "error": str(exc),
        }
        ...
        return {
            "mode": "TIME_SPEC",
            "bandwidth_mhz": 200,
            "classification": "STAGE27G_TIME_SPEC_200_REJECT_FAIL",
            "ok": False,
    ...
    return {
        "mode": "TIME_SPEC",
        "bandwidth_mhz": 200,
        "classification": "STAGE27G_TIME_SPEC_200_NOT_REJECTED",
        "ok": False,
        "error": "configure_science_27g accepted TIME_SPEC at 200MHz",
    }
...
def main() -> int:
    ...
    default_output = _repo_root() / "reports" / "board" / "stage27g_time_spec_100mhz_board.json"
    parser = argparse.ArgumentParser(description="Stage 27g TIME_SPEC 100MHz production convergence board validation.")
    ...
    for index, (mode, bandwidth_mhz) in enumerate(_parse_cases(args.matrix)):
        validations.append(
            core.run_stage27g_time_spec_convergence_validation(
                configure=True,
    ...
    classification = "STAGE27G_TIME_SPEC_100MHZ_BOARD_PASS" if ok else "STAGE27G_TIME_SPEC_100MHZ_BOARD_FAIL"
    result = {
        ...
        "production_scope": dict(core.STAGE27G_PRODUCTION_SCOPE),
        "science_data_scope": "Stage 27g production TIME/SPEC science streams plus Jupyter control/preview",
