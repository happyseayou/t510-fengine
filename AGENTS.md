...
- 耗时较长的仿真（如多帧、多通道真实 XFFT/IP 仿真）也属于长任务，无需用户另行点名，
  同样遵守以下完整提交、健康启动后交还控制权及恢复检查规则。
...
- RTL、Board Agent 和 `t510_time_rx` 只维护并发布 `current`，不得保留运行时版本目录或自动
  回退旧组件；发布失败时停止新组件、保留现场并继续修复当前版本。
- Stage 专属脚本只放在 `scripts/stage-<编号>/`。`scripts/` 根目录只放可跨 Stage 复用的
  构建、发布、资格验证和工具脚本，标准流程不得依赖 Stage 目录。
- 历史 Stage 数据与报告只读保留；current 组件、接口、运行目录和标准文档不得使用 Stage
  编号作为产品身份。
