...
module spec_udp_cmac512 #(
    ...
    localparam integer TOK_CHAN0_LSB       = 288;
    localparam integer TOK_CHAN_COUNT_LSB  = 320;
    localparam integer TOK_TIME_COUNT_LSB  = 336;
    localparam integer TOK_BOARD_LSB       = 352;
    localparam integer TOK_EPOCH_LSB       = 368;
    localparam integer TOK_FLAGS_LSB       = 384;
    localparam integer TOK_QUANT_LSB       = 400;
    localparam integer TOK_SCALE_ID_LSB    = 416;
    localparam integer TOK_NCHAN_LSB       = 448;
    localparam integer TOK_TAPS_LSB        = 464;
    localparam integer TOK_FFT_SHIFT_LSB   = 480;
    localparam integer TOK_SCALE_MODE_LSB  = 496;
    localparam integer TOK_STATUS_LSB      = 512;
    localparam integer TOK_SAMPLE_RATE_LSB = 544;
    localparam integer TOK_CHAN_SPLIT_LSB  = 576;
    localparam integer TOK_SRC_MAC_LSB     = 608;
    localparam integer TOK_SRC_IP_LSB      = 656;
    localparam integer TOK_DST_MAC_LSB     = 688;
    localparam integer TOK_DST_IP_LSB      = 736;
    localparam integer TOK_SRC_PORT_LSB    = 768;
    ...
    logic [31:0] packet_chan_end_comb;
    logic [31:0] route_chan_end_comb;
    ...
    always_comb begin
        ...
        token_din[TOK_SCALE_ID_LSB +: 32] = scale_id;
        token_din[TOK_NCHAN_LSB +: 16] = spec_nchan;
        token_din[TOK_TAPS_LSB +: 16] = spec_taps;
        token_din[TOK_FFT_SHIFT_LSB +: 16] = spec_fft_shift;
        token_din[TOK_SCALE_MODE_LSB +: 16] = scale_mode;
        token_din[TOK_STATUS_LSB +: 32] = spec_status_flags;
        token_din[TOK_SAMPLE_RATE_LSB +: 32] = spec_sample_rate_hz;
    ...
    assign spec_route_hit_count_vec = spec_hit_counts;
    ...
    function automatic [31:0] token_chan0(input [TOKEN_W-1:0] token);
        token_chan0 = token[TOK_CHAN0_LSB +: 32];
    endfunction
    function automatic [15:0] token_chan_count(input [TOKEN_W-1:0] token);
        token_chan_count = token[TOK_CHAN_COUNT_LSB +: 16];
    endfunction
    ...
    function automatic [31:0] token_scale_id(input [TOKEN_W-1:0] token);
        token_scale_id = token[TOK_SCALE_ID_LSB +: 32];
    endfunction
    function automatic [15:0] token_nchan(input [TOKEN_W-1:0] token);
        token_nchan = token[TOK_NCHAN_LSB +: 16];
    endfunction
    function automatic [15:0] token_taps(input [TOKEN_W-1:0] token);
        token_taps = token[TOK_TAPS_LSB +: 16];
    endfunction
    function automatic [15:0] token_fft_shift(input [TOKEN_W-1:0] token);
        token_fft_shift = token[TOK_FFT_SHIFT_LSB +: 16];
    endfunction
    function automatic [15:0] token_scale_mode(input [TOKEN_W-1:0] token);
        token_scale_mode = token[TOK_SCALE_MODE_LSB +: 16];
    endfunction
    function automatic [31:0] token_status(input [TOKEN_W-1:0] token);
        token_status = token[TOK_STATUS_LSB +: 32];
    endfunction
    function automatic [31:0] token_sample_rate(input [TOKEN_W-1:0] token);
        token_sample_rate = token[TOK_SAMPLE_RATE_LSB +: 32];
    endfunction
    function automatic [31:0] token_chan_split(input [TOKEN_W-1:0] token);
        token_chan_split = token[TOK_CHAN_SPLIT_LSB +: 32];
    endfunction
    ...
    function automatic [15:0] token_block_count(input [TOKEN_W-1:0] token);
        begin
            token_block_count = block_count_from_pow2_chan_count(token_nchan(token), token_chan_count(token));
        end
    endfunction

    function automatic [15:0] token_block_index(input [TOKEN_W-1:0] token);
        begin
            token_block_index = block_index_from_pow2_chan_count(token_chan0(token), token_chan_count(token));
        end
    endfunction
    ...
    function automatic [63:0] t510_word_from_token(
        ...
        begin
            case (idx)
                ...
                6:  t510_word_from_token = {token_seq(token), token_chan0(token)};
                7:  t510_word_from_token = {token_chan_count(token), token_time_count(token), LOCAL_NINPUT, token_quant(token)};
                8:  t510_word_from_token = {token_scale_id(token), PAYLOAD_BYTES32};
