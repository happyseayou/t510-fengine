module feng_ctrl_axi #(
    ...
    input  wire [31:0]                  scheduled_sync_error,
    input  wire [63:0]                  scheduled_sync_active_generation,
    input  wire [63:0]                  scheduled_sync_actual_commit_pps_count,
    input  wire [63:0]                  scheduled_sync_actual_epoch_raw_sample0,
    input  wire [63:0]                  scheduled_sync_actual_first_time_sample0,
    input  wire [63:0]                  scheduled_sync_actual_first_spec_sample0,
    ...
    output logic                        scheduled_sync_prepare_pulse,
    output logic                        scheduled_sync_arm_pulse,
    output logic                        scheduled_sync_abort_pulse,
    output logic                        scheduled_sync_clear_status_pulse,
    output logic [63:0]                 scheduled_sync_generation,
    output logic [63:0]                 scheduled_sync_target_pps_count,
    output logic [63:0]                 scheduled_sync_epoch_tai_seconds,
    output logic [63:0]                 scheduled_sync_first_sample0,
    output logic [63:0]                 scheduled_sync_observation_tag,
    output logic [31:0]                 scheduled_sync_signal_chain_tag,
    output logic [31:0]                 scheduled_sync_schedule_tag,
    output logic [31:0]                 mts_result_id,
    ...
    output logic [14:0]                 pfb_coeff_index,
    ...
    output wire [1:0]                   science_sample_rate_mode_cfg,
    output wire [2:0]                   science_output_mode_cfg
);

    localparam [31:0] CORE_VERSION = 32'h0001_0035;
    ...
    logic [1:0]  science_sample_rate_mode;
    ...
    logic [14:0] pfb_coeff_next_index;
    ...
    assign science_sample_rate_mode_cfg = science_sample_rate_mode;
    ...
    function automatic logic reserved_ctrl_addr(input logic [17:0] addr);
        begin
            reserved_ctrl_addr =
                ((addr >= 18'h00400) && (addr < 18'h00440)) ||
                ((addr >= 18'h006e0) && (addr < 18'h006f4)) ||
                ((addr >= 18'h00730) && (addr < 18'h00800)) ||
                ((addr >= 18'h00790) && (addr < 18'h00800)) ||
                ((addr >= 18'h00800) && (addr < 18'h00900)) ||
                ((addr >= 18'h00980) && (addr < 18'h02800)) ||
                (addr == 18'h00378) || (addr == 18'h0037c) ||
                ((addr >= 18'h00380) && (addr < 18'h00400)) ||
                ((addr >= 18'h0a800) && (addr < 18'h0ac00)) ||
    ...
    function automatic [3:0] read_bank_for_addr(input logic [17:0] addr);
        begin
            if (((addr >= 18'h00000) && (addr < 18'h00300)) ||
                ((addr >= 18'h0340) && (addr < 18'h0364))) begin
                read_bank_for_addr = READ_BANK_CORE;
            end else if ((addr >= 18'h00300) && (addr < 18'h00380)) begin
                read_bank_for_addr = READ_BANK_STREAM;
            end else if (((addr >= 18'h00440) && (addr < 18'h0044c)) ||
                         ((addr >= 18'h00600) && (addr < 18'h006e0))) begin
                read_bank_for_addr = READ_BANK_DAC;
            end else if ((addr >= 18'h00500) && (addr < 18'h00540)) begin
                read_bank_for_addr = READ_BANK_LANE_MON;
            end else if ((addr >= 18'h00700) && (addr < 18'h00730)) begin
                read_bank_for_addr = READ_BANK_PREVIEW_CTRL;
                ...
                read_bank_for_addr = READ_BANK_TX;
            end else if ((addr >= 18'h0b100) && (addr < 18'h0b160)) begin
                read_bank_for_addr = READ_BANK_TX_INDIRECT;
            end else if ((addr >= 18'h0d000) && (addr < 18'h0d05c)) begin
                read_bank_for_addr = READ_BANK_SCIENCE;
            end else begin
                read_bank_for_addr = READ_BANK_ZERO;
            end
        end
    endfunction
    ...
    always_ff @(posedge s_axi_aclk or negedge s_axi_aresetn) begin
        if (!s_axi_aresetn) begin
            ...
            scheduled_sync_prepare_pulse <= 1'b0;
            scheduled_sync_arm_pulse <= 1'b0;
            scheduled_sync_abort_pulse <= 1'b0;
            ...
            pfb_coeff_requested_taps <= 4'd8;
            pfb_coeff_index    <= 15'd0;
            pfb_coeff_next_index <= 15'd0;
            pfb_coeff_data     <= 18'sd0;
            pfb_coeff_id       <= 32'h34a8_0001;
            ...
            if (write_exec_valid && !s_axi_bvalid) begin
                if (!reserved_ctrl_addr(write_exec_addr)) begin
                    ...
                    16'h090c: pfb_taps <= 16'd8;
                    16'h0910: pfb_fft_shift <= {4'd0, write_exec_data[11:0]};
                    16'h0914: pfb_chan0 <= 32'd0;
                    16'h0918: begin
                        pfb_chan_count  <= 16'd256;
                        spec_chan_count <= 16'd256;
                    end
                    ...
                    16'h0968: begin
                        pfb_coeff_index <= write_exec_data[14:0];
                        pfb_coeff_next_index <= write_exec_data[14:0];
                    end
                    16'h096c: begin
                        ...
                        if (pfb_coeff_auto_increment) begin
                            pfb_coeff_next_index <= pfb_coeff_next_index + 15'd1;
                        end
                    end
                    16'h0974: pfb_coeff_id <= write_exec_data;
                    16'hd000: begin
                    ...
                    16'hd008: begin
                        case (write_exec_data[1:0])
                            2'd1,
                            2'd2: science_sample_rate_mode <= write_exec_data[1:0];
                            default: science_sample_rate_mode <= 2'd1;
                        endcase
                    end
                    16'hd00c: begin
                    ...
                    16'hac04: begin
                        if (write_exec_data[0]) begin
                            scheduled_sync_prepare_pulse <= 1'b1;
                        end
                        if (write_exec_data[1]) begin
                            scheduled_sync_arm_pulse <= 1'b1;
                        end
                        if (write_exec_data[2]) begin
                            scheduled_sync_abort_pulse <= 1'b1;
                        end
                        if (write_exec_data[3]) begin
                            scheduled_sync_clear_status_pulse <= 1'b1;
                        end
                    end
                    16'hac10: scheduled_sync_generation[31:0] <= write_exec_data;
                    16'hac14: scheduled_sync_generation[63:32] <= write_exec_data;
                    16'hac18: scheduled_sync_target_pps_count[31:0] <= write_exec_data;
                    16'hac1c: scheduled_sync_target_pps_count[63:32] <= write_exec_data;
                    16'hac20: scheduled_sync_epoch_tai_seconds[31:0] <= write_exec_data;
                    16'hac24: scheduled_sync_epoch_tai_seconds[63:32] <= write_exec_data;
                    16'hac28: scheduled_sync_first_sample0[31:0] <= write_exec_data;
                    16'hac2c: scheduled_sync_first_sample0[63:32] <= write_exec_data;
                    16'hac30: scheduled_sync_observation_tag[31:0] <= write_exec_data;
                    16'hac34: scheduled_sync_observation_tag[63:32] <= write_exec_data;
                    16'hac38: scheduled_sync_signal_chain_tag <= write_exec_data;
                    16'hac3c: scheduled_sync_schedule_tag <= write_exec_data;
                    16'hac40: mts_result_id <= write_exec_data;
                    16'hb700: begin
            ...
            if (ar_accept) begin
                ...
                read_bank_latched <= read_bank_for_addr(ar_accept_addr);
                read_pending   <= 1'b1;
    ...
    always_comb begin
        ...
        lane_idx       = 0;
            ...
            case (read_bank_latched)
                ...
                READ_BANK_SYNC: begin
                    case (read_addr)
                        16'hac00: read_data_next = 32'h0102_021f;
                        16'hac04: read_data_next = 32'd0;
                        16'hac08: read_data_next = scheduled_sync_status;
                        16'hac0c: read_data_next = scheduled_sync_error;
                        16'hac10: read_data_next = scheduled_sync_generation[31:0];
                        16'hac14: read_data_next = scheduled_sync_generation[63:32];
                        16'hac18: read_data_next = scheduled_sync_target_pps_count[31:0];
                        16'hac1c: read_data_next = scheduled_sync_target_pps_count[63:32];
                        16'hac20: read_data_next = scheduled_sync_epoch_tai_seconds[31:0];
                        16'hac24: read_data_next = scheduled_sync_epoch_tai_seconds[63:32];
                        16'hac28: read_data_next = scheduled_sync_first_sample0[31:0];
                        16'hac2c: read_data_next = scheduled_sync_first_sample0[63:32];
                        16'hac30: read_data_next = scheduled_sync_observation_tag[31:0];
                        16'hac34: read_data_next = scheduled_sync_observation_tag[63:32];
                        16'hac38: read_data_next = scheduled_sync_signal_chain_tag;
                        16'hac3c: read_data_next = scheduled_sync_schedule_tag;
                        16'hac40: read_data_next = mts_result_id;
                        16'hac44: read_data_next = scheduled_sync_active_generation[31:0];
                        16'hac48: read_data_next = scheduled_sync_active_generation[63:32];
                        16'hac4c: read_data_next = scheduled_sync_actual_commit_pps_count[31:0];
                        16'hac50: read_data_next = scheduled_sync_actual_commit_pps_count[63:32];
                        16'hac54: read_data_next = scheduled_sync_actual_epoch_raw_sample0[31:0];
                        16'hac58: read_data_next = scheduled_sync_actual_epoch_raw_sample0[63:32];
                        16'hac5c: read_data_next = scheduled_sync_actual_first_time_sample0[31:0];
                        16'hac60: read_data_next = scheduled_sync_actual_first_time_sample0[63:32];
                        16'hac64: read_data_next = scheduled_sync_actual_first_spec_sample0[31:0];
                        16'hac68: read_data_next = scheduled_sync_actual_first_spec_sample0[63:32];
                ...
                READ_BANK_FENGINE: begin
                    case (read_addr)
                        ...
                        16'h0968: read_data_next = {17'd0, pfb_coeff_next_index};
                ...
                READ_BANK_SCIENCE: begin
                    case (read_addr)
                        ...
                        16'hd008: read_data_next = {30'd0, science_sample_rate_mode};
                        16'hd00c: read_data_next = {29'd0, science_output_mode};
                        16'hd010: read_data_next = science_sample_rate_hz(science_sample_rate_mode);
                        16'hd014: read_data_next = science_decim_factor(science_sample_rate_mode);
                        16'hd018: read_data_next = science_payload_rate_mbps(science_sample_rate_mode, science_output_mode);
